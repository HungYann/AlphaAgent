      ...
      <div className="auth-stage">
        <div className="auth-panel auth-panel-copy">
          <div className="auth-kicker">
            ...
            <span>{language === 'zh' ? '本地部署' : 'Self-Hosted'}</span>
          </div>
          <h1 className="auth-hero-title">
            {language === 'zh' ? '本地 AI 交易看板' : 'Local AI Trading Dashboard'}
          </h1>
          <p className="auth-hero-copy">
            {language === 'zh'
              ? '完全运行在本地，无需账号。支持 Claude、GPT-4o、DeepSeek，或使用内置巴菲特策略。'
              : 'Runs entirely on your machine. No account needed. Supports Claude, GPT-4o, DeepSeek, or the built-in Buffett skill.'}
          </p>
...
export function RegisterPage({ onLogin }: { onLogin: (token: string) => void }) {
  ...
  return (
    <AuthShell
      ...
      footer={
        <p style={{ textAlign: 'center', color: 'var(--text-secondary)', fontSize: '14px' }}>
          {language === 'zh' ? 'Self-Hosted · 本地运行' : 'Self-Hosted · Running locally'}
        </p>
      }
    >
      <form onSubmit={handleSubmit}>
